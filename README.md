...
| 4. Optimize Combined score | In progress | 1M hot sum **1.32s** (0.60× ClickHouse **2.21s** on same slice) — [`compare-hot.md`](docs/benchmarks/parquet-hits-1m/compare-hot.md) |
...
**Perf (1M Parquet, warm serve):** hot sum **1.32s** vs ClickHouse **2.21s** (**0.60×**). See [`compare-hot.md`](docs/benchmarks/parquet-hits-1m/compare-hot.md).

1. **Q40 / Q41 / Q36 / Q29** — close remaining gap on heavy GROUP BY (coldrun slower than CH on these)
2. **Scale** — 10M+ slice validation; full 100M only for ClickBench cloud run
3. **ClickBench PR** — `clickbench/coldrun/benchmark.sh` on `c6a.4xlarge`
