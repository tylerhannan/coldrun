...
|--------|---------------|---------------------------|
| **Q1–22 sum** | **46.57s** | ~9.6s |
| **Q24–43 sum** | **136.97s** | ~22.8s |
| **42 queries** (bench; Q23 skipped) | **183.54s** | **~32.4s** |
| **All 43** | **238.698s** | **~32.4s** |

**Commit:** `5288c02` · **Log:** `/data/bench-v2-warm-q36fast.log`
...
| Q23 | 55.2s | 0.61s | block-reader + V2 writer path (full warm @ `5288c02`) |
| Q24 | 49.7s | 0.10s | block-reader + V2 writer path (full warm @ `5288c02`) |
| Q25 | 0.008s | 0.038s | **CR wins** |
| Q29 | 7.9s | 9.6s | **CR wins** |
| Q33–35 | ~15–17s | ~3–4s | multi-column GROUP BY |
| Q36 | 0.53s | 0.25s | fused fast path now consistently selected in full warm |
| Q41 | 8.1s | 0.013s | now the largest remaining ratio outlier in top cluster |
...
- **Full warm bench logs:** `/data/bench-v2-warm-full.log` (`80c09f0`), `/data/bench-v2-warm-full-rerun.log` (`657f98d`), `/data/bench-v2-warm-q36check.log` (`51a8b61`), `/data/bench-v2-warm-q36fast.log` (`5288c02`)
- **Data size:** `14176149601` bytes reported by full warm bench (`~14.2 GB`)

| Query | Pre-V2 hot | V2 targeted (`c107ad4`) | V2 full warm (`5288c02`) |
|-------|------------|--------------------------|--------------------------|
| Q23 | 222.341s | **54.501s** | **55.158s** |
| Q24 | 231.3s | **48.673s** | **49.663s** |
| Q36 | 83.271s | n/a | **0.527s** |

Full all-43 hot sum moved from **669.4s** to **238.698s**.
